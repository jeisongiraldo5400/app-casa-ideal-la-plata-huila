-- ============================================================================
-- Agregar filtro por estado a get_purchase_orders_dashboard
-- ============================================================================
...
COMMENT ON FUNCTION public.get_purchase_orders_dashboard(text, integer, integer, text) IS
    'Devuelve órdenes de compra activas (deleted_at IS NULL) con proveedor, items, estado de completitud y total_count. Permite filtrar por estado mediante status_filter (pending, approved, received, cancelled).';

-- ============================================================================
-- Actualizar función RPC: get_purchase_orders_stats para incluir cancelled
-- ============================================================================

-- Eliminar la función existente para poder cambiar el tipo de retorno
DROP FUNCTION IF EXISTS public.get_purchase_orders_stats();

-- Crear la función con el nuevo tipo de retorno que incluye cancelled
CREATE FUNCTION public.get_purchase_orders_stats()
RETURNS TABLE (
    total bigint,
    pending bigint,
    approved bigint,
...
COMMENT ON FUNCTION public.get_purchase_orders_stats() IS
    'Calcula estadísticas globales de órdenes de compra activas (deleted_at IS NULL) en una sola consulta, incluyendo conteo de canceladas.';
