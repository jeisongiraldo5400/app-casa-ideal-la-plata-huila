-- ============================================================================
-- Actualizar RPC get_purchase_orders_dashboard para incluir order_number
-- ============================================================================

-- Eliminar la función existente para poder cambiar el tipo de retorno
DROP FUNCTION IF EXISTS public.get_purchase_orders_dashboard(text, integer, integer, text);

-- Crear la función con el nuevo tipo de retorno que incluye order_number
CREATE FUNCTION public.get_purchase_orders_dashboard(
...
COMMENT ON FUNCTION public.get_purchase_orders_dashboard(text, integer, integer, text) IS
    'Devuelve órdenes de compra activas (deleted_at IS NULL) con proveedor, items, estado de completitud, order_number y total_count. Permite filtrar por estado mediante status_filter (pending, approved, received, cancelled).';
