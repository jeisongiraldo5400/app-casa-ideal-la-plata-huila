...
-- Description: Fix the update_delivery_order_progress function to remove references to updated_at in delivery_order_items
...
BEGIN
  ...
  -- Solo actualizar si la orden no está cancelada

  IF all_items_delivered AND current_order_status != 'cancelled' THEN
...
COMMENT ON FUNCTION public.update_delivery_order_progress(UUID, UUID, INTEGER) IS 
  ...
  'Updates the delivered quantity for a product in a delivery order and automatically marks order as delivered when complete. Fixed to remove updated_at references from delivery_order_items.';
