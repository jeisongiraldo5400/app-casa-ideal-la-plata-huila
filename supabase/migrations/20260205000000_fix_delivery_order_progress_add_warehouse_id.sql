-- ============================================================================
-- Migration: Fix update_delivery_order_progress to filter by warehouse_id
-- Date: 2026-02-05
-- Description: Adds warehouse_id_param to update_delivery_order_progress so it
--              correctly updates only the specific delivery_order_item matching
--              (delivery_order_id, product_id, warehouse_id).
--              Previously, it only matched by (delivery_order_id, product_id),
--              which caused incorrect updates when the same product existed
--              in multiple warehouses within the same delivery order.
--              Also fixes the RETURNING INTO issue with multiple affected rows.
-- ============================================================================

-- Drop the old function signature to avoid overload conflicts
DROP FUNCTION IF EXISTS public.update_delivery_order_progress(UUID, UUID, INTEGER);
...
COMMENT ON FUNCTION public.update_delivery_order_progress(UUID, UUID, UUID, INTEGER) IS
  'Updates the delivered quantity for a specific product+warehouse in a delivery order. Automatically marks order as delivered when all items are complete. Now correctly filters by warehouse_id to prevent incorrect updates when the same product exists in multiple warehouses.';
