...
describe('filterCarteraCuotas', () => {
  ...
  it('«Vendedor» filtra por el dueño del cliente, no por el vendedor del negocio', () => {
    // Un negocio registrado por s1 cuyo cliente pertenece a s2: cuenta para s2.
    const mixed = [
      { ...rows[0], sellerId: 's1', customerSellerId: 's2' },
      { ...rows[1], id: 'c9', sellerId: 's2', customerSellerId: 's2' },
      { ...rows[1], id: 'c3', sellerId: null, customerSellerId: null },
    ];
    const base = { filter: 'todas' as const, search: '', days: 15, municipioId: '', today: '2026-08-12' };

    expect(filterCarteraCuotas(mixed, { ...base, customerSellerId: 's2' }).map((row) => row.id)).toEqual(['c1', 'c9']);
    expect(filterCarteraCuotas(mixed, { ...base, customerSellerId: 's1' })).toHaveLength(0);
    expect(filterCarteraCuotas(mixed, base)).toHaveLength(3);
  });
  ...
  it('filtra por gestor de cobro sin conexión', () => {
    const withGestor = [
      { ...rows[0], gestorId: 'g1' },
      { ...rows[1], gestorId: 'g2' },
    ];
    const result = filterCarteraCuotas(withGestor, { filter: 'todas', search: '', days: 15, municipioId: '', gestorId: 'g1', today: '2026-09-01' });
    expect(result.map((row) => row.id)).toEqual(['c1']);
  });

  it('busca por cédula escrita con puntos, por nombre sin tildes y por número de negocio', () => {
    const people = [
      { ...rows[0], customerName: 'MARÍA PÉREZ', customerIdNumber: '1023456', negocioNumero: 20260012 },
      { ...rows[1], customerName: 'Luis', customerIdNumber: '999', negocioNumero: 20260013 },
    ];
    const params = { filter: 'todas' as const, days: 15, municipioId: '', today: '2026-09-01' };
    expect(filterCarteraCuotas(people, { ...params, search: '1.023.456' }).map((r) => r.id)).toEqual(['c1']);
    expect(filterCarteraCuotas(people, { ...params, search: 'maria perez' }).map((r) => r.id)).toEqual(['c1']);
