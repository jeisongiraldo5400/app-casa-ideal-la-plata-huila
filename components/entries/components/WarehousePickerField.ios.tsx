...
export function WarehousePickerField({ warehouseId, warehouses, onWarehouseChange, colors }: WarehousePickerFieldProps) {
  ...
  const displayLabel = useMemo(() => {
    if (warehouseId === null) return 'Seleccione una bodega';
    const w = warehouses.find((x) => x.id === warehouseId);
    return w?.name || 'Seleccione una bodega';
  }, [warehouseId, warehouses]);

  type Row = { kind: 'placeholder' } | { kind: 'warehouse'; id: string; label: string };

  const rows: Row[] = useMemo(
    () => [
      { kind: 'placeholder' as const },
      ...warehouses.map((w) => ({ kind: 'warehouse' as const, id: w.id, label: w.name || 'Sin nombre' })),
    ],
    [warehouses]
  );
        ...
        <Modal
          ...
          <View style={styles.overlay}>
            ...
            <View style={[styles.sheet, { backgroundColor: paper, borderColor: divider }]}>
              <Text style={[styles.sheetTitle, { color: textPrimary }]}>Bodega</Text>
              <FlatList
                data={rows}
                  ...
                  if (item.kind === 'placeholder') {
                    return (
                      <TouchableOpacity
                        ...
                        <Text style={[styles.optionText, { color: textPrimary }]}>Seleccione una bodega</Text>
                        {warehouseId === null && <MaterialIcons name="check" size={20} color={textPrimary} />}
                      </TouchableOpacity>
                    );
                  }
                  const selected = warehouseId === item.id;
                  return (
                    <TouchableOpacity
                      style={[styles.option, { borderBottomColor: divider }]}
                      onPress={() => {
                        onWarehouseChange(item.id);
                        setOpen(false);
                      }}>
