...
function CollectionRoutesScreenInner() {
  ...
  const todayValue = bogotaDateValue();
  const { today: todayRoute, upcoming, unfinished, history } = groupRoutesForHome(routes, todayValue);
  const tomorrowValue = addDaysToDateValue(todayValue, 1);
  const hasTomorrow = upcoming.some((route) => route.route_date === tomorrowValue);
  return (
    <ScrollView style={{ backgroundColor: colors.background.default }} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={loading} onRefresh={load} />}>
      ...
      {todayRoute ? (
        ...
        <TouchableOpacity
          ...
          <Text style={[styles.cardTitle, { color: colors.text.primary }]}>Crear ruta</Text>
          <Text style={{ color: colors.text.secondary, textAlign: 'center' }}>
            {online
              ? 'Para hoy, mañana u otra fecha. Elige entre todos tus negocios asignados y ordena las visitas.'
              : 'Necesitas conexión para crear una ruta.'}
          </Text>
        </TouchableOpacity>
      )}

      {todayRoute && !hasTomorrow ? (
        <TouchableOpacity
          accessibilityRole="button"
          disabled={!online}
          accessibilityState={{ disabled: !online }}
          style={[styles.planCard, { borderColor: online ? colors.primary.main : colors.divider, opacity: online ? 1 : 0.6 }]}
          onPress={() => router.navigate(`/(tabs)/ruta-cobros-crear?fecha=${tomorrowValue}` as any)}>
          <MaterialIcons name="event" size={22} color={online ? colors.primary.main : colors.text.secondary} />
          <Text style={{ color: colors.text.primary, fontWeight: '800', flex: 1 }}>Armar la ruta de mañana</Text>
      ...
      {upcoming.map((route) => (
        <RouteCard
          key={route.id}
          route={route}
          title={route.route_date === tomorrowValue ? 'Ruta de mañana' : `Ruta del ${routeDateLabel(route.route_date)}`}
          hint="Programada: iníciala ese día."
          colors={colors}
      ...
      {history.map((route) => (
        <TouchableOpacity key={route.id} style={[styles.historyCard, { backgroundColor: colors.background.paper, borderColor: colors.divider }]} onPress={() => router.push(`/ruta-cobros/${route.id}` as any)}>
          ...
          <View style={{ flex: 1 }}><Text style={{ color: colors.text.primary, fontWeight: '800' }}>{routeDateLabel(route.route_date)}</Text><Text style={{ color: colors.text.secondary }}>{route.completed_count}/{route.stop_count} visitas{notVisitedSuffix(route)} · {formatCOP(route.collected_total)}</Text></View>
  ...
  return (
    <TouchableOpacity accessibilityRole="button" style={[styles.activeCard, { backgroundColor: colors.background.paper }]} onPress={onPress}>
      ...
      <View style={[styles.metrics, { borderTopColor: colors.divider }]}>
        <View><Text style={[styles.metricLabel, { color: colors.text.secondary }]}>Esperado</Text><Text style={[styles.metricValue, { color: colors.text.primary }]}>{formatCOP(route.expected_total)}</Text></View>
        <View><Text style={[styles.metricLabel, { color: colors.text.secondary }]}>Recaudado</Text><Text style={[styles.metricValue, { color: colors.success.main }]}>{formatCOP(route.collected_total)}</Text></View>
      </View>
    </TouchableOpacity>
  );
}
...
const styles = StyleSheet.create({
  ...
  error: { padding: 12, borderRadius: 10 }, offlineNotice: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 12, borderWidth: 1, borderRadius: 12 }, activeCard: { borderRadius: 18, padding: 16, elevation: 2 }, createCard: { borderRadius: 18, borderWidth: 1.5, borderStyle: 'dashed', padding: 26, alignItems: 'center', gap: 8 }, rowBetween: { flexDirection: 'row', alignItems: 'center', gap: 12 }, iconCircle: { width: 48, height: 48, borderRadius: 24, justifyContent: 'center', alignItems: 'center' }, cardTitle: { fontSize: 17, fontWeight: '900' }, metrics: { flexDirection: 'row', justifyContent: 'space-around', borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#cbd5e1', paddingTop: 14, marginTop: 14 }, metricLabel: { color: '#64748b', fontSize: 11, textTransform: 'uppercase', fontWeight: '700' }, metricValue: { fontSize: 16, fontWeight: '900', marginTop: 3 }, sectionTitle: { fontSize: 17, fontWeight: '900', marginTop: 8 }, historyCard: { flexDirection: 'row', alignItems: 'center', gap: 12, borderWidth: 1, borderRadius: 14, padding: 14 }, planCard: { flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderStyle: 'dashed', borderRadius: 14, padding: 14 },
});
